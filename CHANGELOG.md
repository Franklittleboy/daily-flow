...
- 任务分组现在可折叠，并会保留每个分组的展开状态。
- Markdown 子任务支持缩进、层级引导线和后代折叠摘要，复杂任务结构更容易阅读。
- 选中任务使用统一的 TickTick 风格表面，标题可直接行内编辑，子内容任务使用独立标记。
- 任务详情重绘后保留列表滚动位置，任务栏的导航、标题和底部操作保持稳定。
- 整合并验证专注、日历、任务详情和移动端功能合同，发布前流程现在会检查分支基线和共享生产文件。
