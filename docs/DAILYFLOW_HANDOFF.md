...
- Task detail popover:
  ...
  Latest md-detail polish enlarges the task title, reduces the markdown body to a TickTick-like default text size, keeps empty note bodies blank, adds configurable DailyFlow slash commands in settings with defaults for todo/list, divider, and strikethrough insertion, and lets users drag/pan zoomed image attachment previews. Follow-up md editor polish first replaced the raw textarea with a lightweight DailyFlow Live Preview editor: `- [ ]` lines render as checkbox rows, `- item` and `* item` render as unordered bullet rows, `---` renders as a divider, and `~~text~~` renders with strikethrough while still saving markdown text underneath. Latest CodeMirror md editor polish replaces that contentEditable editor with a bundled CodeMirror basic editor so IME composition is not interrupted; the editor keeps DailyFlow slash completions for todo/list, divider, and strikethrough, renders todo boxes/bullets/dividers/strikethrough via CodeMirror decorations, and tightens the detail title/date typography toward the TickTick reference with a small calendar icon and relative date labels such as today, tomorrow, and next weekday. Latest follow-up restores the markdown body to normal weight text and replaces CodeMirror's built-in autocomplete path with a DailyFlow-owned slash menu anchored to the CodeMirror cursor; empty slash command settings now fall back to DailyFlow defaults.
- Mobile layout:
