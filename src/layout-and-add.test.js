...
test("task pages add a TickTick-like list column with a persisted draggable split", () => {
  ...
  assert.match(styles, /\.daily-flow-main\.is-focus\s*{[^}]*padding:\s*22px 30px/s);
...
test("calendar layout gives more width and height to the calendar grid", () => {
  ...
  assert.match(styles, /\.daily-flow-main\.is-calendar \.daily-flow-header\s*{[^}]*margin-bottom:\s*10px/s);
...
test("task detail content matches TickTick-like side panel and calendar popover layout", () => {
  ...
  assert.match(source, /if \(this\.detailDatePickerOpen\) \{\s*card\.appendChild\(this\.renderDetailDatePicker\(task\)\);\s*\}\s*const titleRow = createEl\("div", "daily-flow-detail-title-row"\)/s);
  ...
  assert.match(styles, /\.daily-flow-task-list-scroll\s*{[^}]*overflow-x:\s*hidden/s);
  ...
  assert.match(styles, /\.daily-flow-detail-title-row\s*{[^}]*padding:\s*28px 40px 22px/s);
  assert.match(styles, /\.daily-flow-detail-content\s*{[^}]*padding:\s*0 40px 20px/s);
  ...
  assert.match(styles, /\.daily-flow-detail-md-body\s*{[^}]*overflow:\s*visible/s);
